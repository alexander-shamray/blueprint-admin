...
class TheShippedMap(unittest.TestCase):
    ...
    def test_class_d_reaches_no_code(self) -> None:
        ...
        skipped = {"bin", "obj", "TestResults", "node_modules", "wwwroot", ".angular"}
        code = [
    ...
    def test_a_class_a_change_to_claude_md_is_caught(self) -> None:
        problems = check(payload(["src/Admin.Host/Program.cs", "CLAUDE.md"],
                                 touch_cell="`src/Admin.Host/**`, `CLAUDE.md`"), self.map)
        self.assertEqual(len(problems), 1)
        self.assertIn("CLAUDE.md", problems[0])

    def test_a_class_a_change_touching_two_slices_is_caught_by_the_declared_set(self) -> None:
        problems = check(payload(["src/Admin.Host/Program.cs", "src/Admin.Web/src/main.ts"],
                                 touch_cell="`src/Admin.Host/**`, `tests/Admin.Host.Tests/**`"), self.map)
        self.assertEqual(len(problems), 1)
        self.assertIn("src/Admin.Web/src/main.ts", problems[0])
