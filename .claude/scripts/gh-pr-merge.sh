...
# ancestors of `main`. `ship.md` step 0's finished predicate reads `git cherry`
# — patch-ids — for that reason, and moving this flag without moving that read
# leaves every delivered worktree standing for ever.
set -euo pipefail
[ "$#" -eq 2 ] ||
